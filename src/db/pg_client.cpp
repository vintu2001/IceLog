...
bool PgClient::create_table(PGconn *conn, const std::string &table_name,
  ...
  const char *params[] = {table_name.c_str(), schema_json.c_str(),
                          partition_spec.c_str(), properties_json.c_str()};

  PGresult *res = PQexecParams(conn,
                               "INSERT INTO tables (table_name, schema_json, "
                               "partition_spec, properties) "
                               "VALUES ($1, $2::jsonb, $3, $4::jsonb) ON "
  ...
  // Check if any row was actually inserted (0 means duplicate was skipped)
...
std::optional<TableRow> PgClient::get_table(PGconn *conn,
...
bool PgClient::update_table_schema(PGconn *conn, const std::string &table_name,
  ...
  const char *params1[] = {new_schema_json.c_str(), ver_str.c_str(),
                           table_name.c_str()};
  PGresult *res = PQexecParams(
      conn,
  ...
  const char *params2[] = {tid_str.c_str(), ver_str.c_str(),
                           new_schema_json.c_str(), change_summary.c_str()};
  res = PQexecParams(conn,
                     "INSERT INTO schema_history (table_id, schema_version, "
                     "schema_json, change_summary) "
...
bool PgClient::drop_table(PGconn *conn, const std::string &table_name,
                          bool purge) {
  if (purge) {
    int64_t table_id = get_table_id(conn, table_name);
    if (table_id < 0)
    ...
    PQexecParams(conn, "DELETE FROM partitions WHERE table_id = $1", 1, nullptr,
                 p, nullptr, nullptr, 0);
    PQexecParams(conn, "DELETE FROM snapshots WHERE table_id = $1", 1, nullptr,
                 p, nullptr, nullptr, 0);
    PQexecParams(conn, "DELETE FROM schema_history WHERE table_id = $1", 1,
                 nullptr, p, nullptr, nullptr, 0);

    const char *params[] = {table_name.c_str()};
    PGresult *res =
        PQexecParams(conn, "DELETE FROM tables WHERE table_name = $1", 1,
    ...
    PGresult *res =
        PQexecParams(conn,
                     "UPDATE tables SET is_deleted = true, updated_at = now() "
...
std::vector<TableRow> PgClient::list_tables(PGconn *conn,
                                            const std::string & /*ns*/,
                                            int32_t page_size,
  ...
  PGresult *res = PQexecParams(
      ...
      "SELECT table_id, table_name, schema_json::text, schema_version, "
      "       partition_spec, current_snapshot_id, properties::text "
      "FROM tables WHERE is_deleted = false ORDER BY table_id LIMIT $1 OFFSET "
      "$2",
      2, nullptr, params, nullptr, nullptr, 0);
...
PgClient::query_partitions(PGconn *conn, const std::string &table_name,
  ...
  PGresult *res =
      PQexecParams(conn,
                   ...
                   "       data_file_path, file_format, row_count, size_bytes, "
                   "column_stats::text "
                   "FROM partitions "
...
std::optional<std::vector<PartitionRow>>
PgClient::query_partitions_paged(PGconn *conn, const std::string &table_name,
  ...
  const char *params[] = {tid_str.c_str(), snap_str.c_str(), last_str.c_str(),
                          limit_str.c_str()};
...
bool PgClient::insert_partition(PGconn *conn, const std::string &table_name,
  ...
  const char *params[] = {
      tid_str.c_str(),
      snap_str.c_str(),
      part.partition_key.c_str(),
      part.data_file_path.c_str(),
      part.file_format.c_str(),
      rc_str.c_str(),
      sz_str.c_str(),
  ...
  PGresult *res =
      PQexecParams(conn,
                   "INSERT INTO partitions (table_id, snapshot_id, "
                   "partition_key, data_file_path, "
...
bool PgClient::mark_partition_deleted(PGconn *conn,
                                      const std::string &table_name,
  ...
  const char *params[] = {snap_str.c_str(), tid_str.c_str(),
                          partition_key.c_str()};
...
uint64_t PgClient::insert_snapshot(PGconn *conn, const std::string &table_name,
  ...
  const char *params[] = {tid_str.c_str(), par_str.c_str(), operation.c_str(),
...
uint64_t PgClient::get_current_snapshot(PGconn *conn,
                                        const std::string &table_name) {
  const char *params[] = {table_name.c_str()};
  PGresult *res = PQexecParams(conn,
                               "SELECT current_snapshot_id FROM tables WHERE "
                               "table_name = $1 AND is_deleted = false",
                               1, nullptr, params, nullptr, nullptr, 0);
...
bool PgClient::update_table_snapshot(PGconn *conn,
                                     const std::string &table_name,
                                     uint64_t snapshot_id) {
  std::string snap_str = std::to_string(snapshot_id);
...
bool PgClient::update_transaction_status(PGconn *conn, uint64_t txn_id,
  ...
  PGresult *res = PQexecParams(conn, query.c_str(), 2, nullptr, params, nullptr,
                               nullptr, 0);
