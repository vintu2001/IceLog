...
MetadataServer::MetadataServer(const ServerConfig &config)
    : pg_pool_(std::make_unique<ConnectionPool>(config.pg_conn_string,
                                                config.pool_size)),
      mvcc_manager_(std::chrono::seconds(config.txn_timeout_s)) {
  catalog_ = std::make_unique<CatalogManager>(*pg_pool_, pg_client_,
                                              lock_manager_, mvcc_manager_);
  partitions_ =
      std::make_unique<PartitionRegistry>(*pg_pool_, pg_client_, lock_manager_,
                                          mvcc_manager_, config.cache_capacity);
  schemas_ =
      std::make_unique<SchemaStore>(*pg_pool_, pg_client_, lock_manager_);
...
grpc::Status
MetadataServer::CreateTable(grpc::ServerContext * /*ctx*/,
...
grpc::Status
MetadataServer::GetTableMetadata(grpc::ServerContext * /*ctx*/,
  ...
  // If MVCC counter is still 0, use the DB-stored snapshot from the table
  if (read_snapshot == 0) {
    read_snapshot = static_cast<uint64_t>(table->current_snapshot_id);
  }
...
grpc::Status MetadataServer::AlterTable(grpc::ServerContext * /*ctx*/,
  ...
  case metadata::AlterTableRequest::kNewSchemaJson: {
    ...
    if (current.has_value()) {
      auto err = schemas_->validate_schema_change(current->schema_json,
                                                  req->new_schema_json());
      if (!err.empty()) {
...
grpc::Status MetadataServer::DropTable(grpc::ServerContext * /*ctx*/,
...
grpc::Status MetadataServer::ListTables(grpc::ServerContext * /*ctx*/,
                                        const metadata::ListTablesRequest *req,
                                        metadata::ListTablesResponse *resp) {
  auto tables = catalog_->list_tables(req->namespace_(), req->page_size(),
                                      req->page_token());
...
MetadataServer::GetPartitions(grpc::ServerContext * /*ctx*/,
  ...
  if (!result.has_value()) {
    return grpc::Status(grpc::NOT_FOUND,
                        "Table not found: " + req->table_name());
  }
...
grpc::Status
MetadataServer::GetPartitionStats(grpc::ServerContext * /*ctx*/,
...
grpc::Status
MetadataServer::CommitSnapshot(grpc::ServerContext * /*ctx*/,
  ...
  std::vector<std::string> deleted_keys(req->deleted_partition_keys().begin(),
                                        req->deleted_partition_keys().end());

  auto result = partitions_->commit_snapshot(
      req->table_name(), req->parent_snapshot_id(),
      req->operation().empty() ? "append" : req->operation(), new_parts,
      deleted_keys);
...
grpc::Status
MetadataServer::GetSnapshot(grpc::ServerContext * /*ctx*/,
                            const metadata::GetSnapshotRequest *req,
                            metadata::SnapshotDetail *resp) {
  auto conn = pg_pool_->acquire();
  auto snap = pg_client_.get_snapshot(conn.get(), req->table_name(),
                                      req->snapshot_id());
  if (!snap.has_value()) {
...
grpc::Status
MetadataServer::ListSnapshots(grpc::ServerContext * /*ctx*/,
                              const metadata::ListSnapshotsRequest *req,
                              metadata::ListSnapshotsResponse *resp) {
  auto conn = pg_pool_->acquire();
  auto snapshots =
      pg_client_.list_snapshots(conn.get(), req->table_name(), req->limit());
...
grpc::Status
MetadataServer::BeginTransaction(grpc::ServerContext * /*ctx*/,
  ...
  std::string isolation = (req->isolation() == metadata::READ_COMMITTED)
                              ? "read_committed"
                              : "snapshot";
  pg_client_.insert_transaction(conn.get(), req->client_id(), read_snapshot,
                                isolation);
...
grpc::Status
MetadataServer::CommitTransaction(grpc::ServerContext * /*ctx*/,
  ...
  if (ok) {
    auto conn = pg_pool_->acquire();
    pg_client_.update_transaction_status(conn.get(), req->txn_id(),
                                         "committed");
  }
...
grpc::Status
MetadataServer::AbortTransaction(grpc::ServerContext * /*ctx*/,
